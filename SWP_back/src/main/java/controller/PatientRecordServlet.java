...
@WebServlet("/api/patient_records/*")
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (patientIdParam != null && !patientIdParam.isEmpty()) {
                ...
                List<PatientRecordDTO> records = patientRecordDAO.getPatientRecordsByPatientId(patientId);

                // Return only records for patientId (no pagination)
                Map<String, Object> responseData = new HashMap<>();
                responseData.put("records", records);
                String json = gson.toJson(responseData);
                response.getWriter().write(json);
            } else if (accountPatientIdParam != null && !accountPatientIdParam.isEmpty()) {
                ...
                List<PatientRecordDTO> records = patientRecordDAO.getPatientRecordsByAccountIdWithFilters(
                        accountPatientId, patientName,
                        startDate, endDate, gender, page, size);
                int totalRecords = patientRecordDAO.countPatientRecordsByAccountIdWithFilters(
                        accountPatientId, patientName,
                ...
                response.getWriter().write("{\"error\": \"Missing patientId or accountPatientId parameter\"}");
            }
        } catch (NumberFormatException e) {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            response.getWriter().write("{\"error\": \"Invalid ID or pagination parameter format\"}");
        } catch (Exception e) {
